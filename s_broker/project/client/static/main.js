...
$('.flush_redis').on('click', function() {
  console.log('button clicked!');
  $.ajax({
    url: master1_url + '/api/flush_redis',
    data: { type: $(this).data('type') },
    method: 'POST'
  })
  .done((res) => {
...
$('.checkQueue').on('click', function() {
  ...
  $.ajax({
    url: master1_url + '/api/checkqueue',
    data: { queue: 'queue' },
    method: 'POST'
  })
  .done((res) => {
...
$('.getAllQueues').on('click', function() {
  $.ajax({
    url: master1_url + '/api/getallqueues',
    method: 'POST'
  })
  .done((res) => {
    console.log(res);
  ...
  .done((res) => {
    ...
    for (var i = 0; i < ftask_length; i++) {
      ...
      html +=  `<td>${ftask[i][t].handled_time}</td>
                ...
                <td>Finished</td>
                <td><a href="${master1_url}/api/task/default/${t}">Link</a></td>`;
      html += '</tr>';
    }
    ...
    for (var i = 0; i < aftask_length; i++) {
      ...
      html +=  `<td>${aftask[i][t].handled_time}</td>
                ...
                <td>Finished</td>
                <td><a href="${master1_url}/api/task/aggregator/${t}">Link</a></td>`;
      html += '</tr>';
    }
...
$('.test_nuts').on('click', function() {
  console.log('button clicked!');
  $.ajax({
    url: master1_url + '/api/nuts_classify',
    data: { type: $(this).data('type') },
    method: 'POST'
  })
  .done((res) => {
